...
$('#life-mode-toggle').addEventListener('click', () => { lifeMode = !lifeMode; const hideInMemory = ['.capture-panel', '.welcome-grid', '.revisit-card', '.search-row', '.tabs', '#notes-section', '#calendar-view']; hideInMemory.forEach((selector) => { const element = $(selector); if (element) element.hidden = lifeMode; }); $('#life-mode').hidden = !lifeMode; $('#life-mode-toggle').textContent = lifeMode ? '記憶デスクへ ✦' : '人生タイマー ◇'; if (lifeMode && window.refreshLifeNative) window.refreshLifeNative(); });
